    ...
    do {
        ...
        while ( (token = strtok_r(line_ptr, delim, &checkpoint)) != NULL) {
            ...
            if (token[0] == ':') {
                ...
                while ( (token = strtok_r(NULL, delim, &checkpoint)) != NULL) {
                    if (strcmp(token, "*/") == 0) {
                        break;
                    }
                }
                int old_line_num = line_num;
                while (token == NULL) { /* While end of comment not found in 
                                         * this line. */
                    if (fgets(line, MAX_LINE_LEN, fp) == NULL) {
                        fprintf(stderr, "\nError: Unexpected end of file-"
                                "End of comment not found for"
                                " comment started in line number %d", 
                                old_line_num);
                        return FAILURE;
                    }
                    line_num++;
 ...
 * @return                     Returns a status_t.
 */
status_t vm_compile_first_pass (bytecode_t *compiled_code, int *len, FILE *fp, 
                                 ...
                                 label_t *label_table, const int lt_len)
{
    ...
    for (i = 0; i < code_len; i++) {
        if (compiled_code[i] == INST_SET[PUSH].bytecode) {
            ...
            label_table[label_count].pc = (bytecode_t)i;
            label_count++;
            i++;
        } 
    }
...
int main (int argc, char *argv[]) 
{
    ...
    rewind(fp);
    if (vm_compile_first_pass(compiled_code, &code_len, fp, label_table, 
                              label_count) == FAILURE) {
